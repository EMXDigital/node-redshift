 ...
 */
function runQuery(that, client, query, parameters, opts, resolve, reject, done) {
  console.log('running RS query', query);
  client.query(query, parameters, function (err, data) {
    if (err) {
      reject(errors.verifyQueryError(err, opts.maxQueryTimeout));
    } else {
