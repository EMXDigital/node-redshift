...
function runQuery(that, client, query, parameters, opts, resolve, reject, done, endConnection) {
  ...
  // Verify if we have any query timeout to restrict the logic running time
  var connectionTimeoutSet = null;
  if (opts && opts.maxQueryTimeout && typeof opts.maxQueryTimeout === 'number') {
    connectionTimeoutSet = setTimeout(function() {
      // release client and exit
      releaseClient(); 
      reject(getTimeoutError(opts.maxQueryTimeout));
    }, opts.maxQueryTimeout);
  }

  client.query(query, parameters, function (err, data) {
    if (connectionTimeoutSet) {
      clearTimeout(connectionTimeoutSet);
    }
 ...
 */
function getTimeoutError(timeout) {
  var err = new Error(`Max connection Query ${(timeout||'default')+' ms'} Timeout Reached`);
  err.type = 'DataQueryTimeout';
  return err;
}
